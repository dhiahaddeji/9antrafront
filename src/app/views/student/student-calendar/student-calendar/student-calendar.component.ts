...
export class StudentCalendarComponent implements OnInit {
  ...
  retrieveSessions(): void {
    ...
    this.sessionService.getSessionsByUserId(userId).subscribe({
      next: (sessions: Session[]) => {
        // precompute derived fields once so templates never call Date() per CD cycle
        const now = new Date();
  ...
  // kept for backward compat (called from selectSession / updateButtonStatus)
  ...
  private updateButtonStatus(): void {
    ...
    const start = this.databaseDate?.getTime();
    const end = start + this.durationInMinutes * 60 * 1000;
    this.isButtonDisabled = now <= start || now >= end;
  }
  ...
  buildCalendar(): void {
    ...
    // build weeks array once here instead of a getter called every CD cycle
